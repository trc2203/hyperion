...
float pid_d_gain_roll = 20;                //Gain setting for the roll D-controller
int pid_max_roll = 400;                    //Maximum output of the PID-controller (+/-)

float pid_p_gain_pitch = 1.8;              //Gain setting for the pitch P-controller.
...
void loop(){
  ...
  angle_pitch += gyro_pitch * 0.0000611;                                    //Calculate the travelled pitch angle and add this to the angle_pitch variable.
  angle_roll += gyro_roll * 0.0000611;                                      //Calculate the travelled roll angle and add this to the angle_roll variable.
  ...
  if(abs(acc_y) < acc_total_vector){                                        //Prevent the asin function producing a NaN
    angle_pitch_acc = asin((float)acc_y/acc_total_vector)* 57.296;          //Calculate the pitch angle.
  }
  if(abs(acc_x) < acc_total_vector){                                        //Prevent the asin function producing a NaN
  ...
  //When yaw stick is back in the centre position start the motors (step 2).
  ...
  pid_pitch_setpoint -= pitch_level_adjust;                                  //Subtract the angle correction from the standardised receiver pitch input value.
  ...
  if (start == 2){                                                          //The motors are started.
    ...
    //Roll output for rear motors removed as they have no effect
    //Yaw output removed for front motors to avoid also causing roll
    esc_1 = throttle - pid_output_pitch + pid_output_roll;                  //Calculate the pulse for esc 1 (front-right - ACW)
    esc_2 = throttle + pid_output_pitch + pid_output_yaw;                   //Calculate the pulse for esc 2 (rear-top - CW)
    esc_3 = throttle + pid_output_pitch - pid_output_yaw;                   //Calculate the pulse for esc 3 (rear-bottom - ACW)
    esc_4 = throttle - pid_output_pitch - pid_output_roll;                  //Calculate the pulse for esc 4 (front-left - CW)

    if (battery_voltage < 1650 && battery_voltage > 1000){                  //Is the battery connected?
      esc_1 += esc_1 * ((1650 - battery_voltage)/(float)2500);              //Compensate the esc-1 pulse for voltage drop.
      esc_2 += esc_2 * ((1650 - battery_voltage)/(float)2500);              //Compensate the esc-2 pulse for voltage drop.
      esc_3 += esc_3 * ((1650 - battery_voltage)/(float)2500);              //Compensate the esc-3 pulse for voltage drop.
      esc_4 += esc_4 * ((1650 - battery_voltage)/(float)2500);              //Compensate the esc-4 pulse for voltage drop.
    } 
  ...
  while(micros() - loop_timer < 4000);                                      //We wait until 4000us have passed.
  loop_timer = micros();                                                    //Set the timer for the next loop.
...
int convert_receiver_channel(byte function){
  ...
  centre = (eeprom_data[channel * 2 - 1] << 8) | eeprom_data[channel * 2 - 2]; //Store the centre value for the specific receiver input channel
  high = (eeprom_data[channel * 2 + 7] << 8) | eeprom_data[channel * 2 + 6];   //Store the high value for the specific receiver input channel

  if(actual < centre){                                                         //The actual receiver value is lower than the centre value
    if(actual < low)actual = low;                                              //Limit the lowest value to the value that was detected during setup
    difference = ((long)(centre - actual) * (long)500) / (centre - low);       //Calculate and scale the actual value to a 1000 - 2000us value
    if(reverse == 1)return 1500 + difference;                                  //If the channel is reversed
    else return 1500 - difference;                                             //If the channel is not reversed
  }
  else if(actual > centre){                                                    //The actual receiver value is higher than the centre value
    if(actual > high)actual = high;                                            //Limit the lowest value to the value that was detected during setup
    difference = ((long)(actual - centre) * (long)500) / (high - centre);      //Calculate and scale the actual value to a 1000 - 2000us value
    if(reverse == 1)return 1500 - difference;                                  //If the channel is reversed
    else return 1500 + difference;                                             //If the channel is not reversed
  }
  else return 1500;
...
void set_gyro_registers(){
  ...
  if(eeprom_data[31] == 1){
    ...
    Wire.endTransmission();                                                    //End the transmission
    Wire.requestFrom(gyro_address, 1);                                         //Request 1 byte from the gyro
